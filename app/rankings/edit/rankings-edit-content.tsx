...
import { LogOut, User, Award, TrendingUp, Plus, Save, Trash2, Eye } from 'lucide-react';
...
export default function RankingsEditContent() {
  ...
  const loadStudents = async () => {
    try {
      ...
      // ALSO check studentAttendance collection for individual records
      const studentAttendanceSnapshot = await getDocs(collection(db, 'studentAttendance'));
      console.log('🔍 Checking studentAttendance collection...', studentAttendanceSnapshot.size, 'records');
      
      studentAttendanceSnapshot.forEach((doc) => {
        const data = doc.data();
        const rollNo = data.rollNo;
        
        if (rollNo && attendanceMap[rollNo]) {
          // Count individual attendance records
          // If record exists and not explicitly marked absent, count as present
          ...
          console.log(`📊 Individual attendance: ${rollNo} on ${data.date} = ${isPresent ? 'Present' : 'Absent'}`);
        }
      });
      ...
      rankingsSnapshot.forEach((doc) => {
        ...
        console.log('🔍 Ranking data for', doc.id, ':', rankingData);
        rankingsMap.set(doc.id, rankingData);
      });

      console.log('🔍 Total ranking records:', rankingsSnapshot.size);
      ...
      studentMap.forEach((student, rollNo) => {
        ...
        console.log('  Task Marks Object:', taskMarks);
        console.log('  Total Task Marks:', totalTaskMarks);
        console.log('  Bonus Marks:', bonusMarks);
        console.log('  TOTAL SCORE:', totalScore);
        console.log('  Breakdown:', attendanceMarks, '+', totalTaskMarks, '+', bonusMarks, '=', totalScore);
  ...
  const loadTasks = async () => {
    try {
      ...
      tasksSnapshot.forEach((doc) => {
        ...
        console.log('🔍 Found task:', taskData.id, taskData.title, 'Active:', taskData.isActive);
        tasksData.push(taskData);
      });
      
      const activeTasks = tasksData.filter(t => t.isActive);
      console.log('🔍 Total tasks loaded:', tasksData.length);
      console.log('🔍 Active tasks:', activeTasks.length, activeTasks.map(t => `${t.title} (${t.id})`));
  ...
  const handleAttendanceChange = (rollNo: string, value: string) => {
    const numValue = Math.max(0, parseInt(value) || 0);

    setStudents(prev => prev.map(student => {
      if (student.rollNo === rollNo) {
        const totalTaskMarks = Object.values(student.taskMarks).reduce((sum, marks) => sum + marks, 0);
        const totalScore = numValue + totalTaskMarks + student.bonusMarks;
        
        return { ...student, attendanceMarks: numValue, totalScore };
      }
      return student;
    }));
  ...
  const handleBulkGiveFullMarks = () => {
    if (selectedStudents.size === 0 || tasks.length === 0) {
      showToast('Please select students and ensure tasks exist', 'error');
      return;
    }

    setStudents(prev => prev.map(student => {
      if (selectedStudents.has(student.rollNo)) {
        const updatedTaskMarks = { ...student.taskMarks };
        
        // Give full marks for all tasks
        tasks.forEach(task => {
          updatedTaskMarks[task.id] = task.maxMarks;
        });
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Edit Student Rankings</h1>
          <p className="text-white/60">
            Manage marks, tasks, and scores • All changes auto-calculate rankings
          </p>
        </div>
          ...
          <>
            ...
            <div className="mb-6 glass-effect-strong border border-white/10 rounded-lg p-4">
              <p className="text-white/60 text-sm">
                ...
                Editable (auto-calculated from present days shown below) • Task Marks = Editable (0-max) • Double-click to
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-center">
                          <input
                            ...
                            className="w-20 px-2 py-1 bg-white/5 border border-white/20 rounded text-white text-center focus:outline-none focus:ring-2 focus:ring-green-500"
                          />
                          <span className="text-white/40 text-xs block mt-1">
                            Auto: {student.presentDays}/{student.totalDays}
                          </span>
                        </td>
                        {tasks.map((task) => (
